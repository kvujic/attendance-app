@extends('layouts.app')

@section('title', '勤怠詳細')

@section('css')
<link rel="stylesheet" href="{{ asset('css/attendance-detail.css') }}">
@endsection

@section('content')

@include('components.user_header')

<div class="attendance-detail__container">
    <h1 class="attendance-detail__title">勤怠詳細</h1>
    <form action="{{ route('attendance.update', ['id' => $id]) }}" method="POST">
        @csrf
        <table class="attendance-detail__table">
            <tbody id="break-rows">
                <tr class="attendance-detail__table-row">
                    <th class="detail-label">名前</th>
                    <td class="detail-data data-name">{{ $user->name }} </td>
                </tr>
                <tr class="attendance-detail__table-row">
                    <th class="detail-label">日付</th>
                    <td class="detail-data">
                        @if ($isPending || $viewReadOnly)
                        <span class="date-display data__input-date year">
                            {{ \Carbon\Carbon::parse($attendance->date)->format('Y年') }}
                        </span>
                        <span class="date-display data__input-date month-date">
                            {{ \Carbon\Carbon::parse($attendance->date)->format('n月j日')}}
                        </span>
                        <input type="hidden" name="date" value="{{ \Carbon\Carbon::parse($attendance->date)->format('Y-m-d') }}">
                        @else
                        <input type="text" class="data__input-date year" value="{{ \Carbon\Carbon::parse($attendance->date)->format('Y年') }}" readonly>
                        <input type="text" class="data__input-date month-date" value="{{ \Carbon\Carbon::parse($attendance->date)->format('n月j日') }}" readonly>
                        <input type="hidden" name="date" value="{{ \Carbon\Carbon::parse($attendance->date)->format('Y-m-d') }}">
                        @endif
                        @error('date')
                        <div class="attendance-detail__error">{{ $message }}</div>
                        @enderror
                    </td>
                </tr>
                <tr class="attendance-detail__table-row">
                    <th class="detail-label">出勤・退勤</th>
                    <td class="detail-data">
                        @if ($isPending || $viewReadOnly)
                        <span class="time-display data__input-time">{{ $requestedClockIn ?? '-' }}</span>
                        <span class="tilde">〜</span>
                        <span class="time-display data__input-time">{{ $requestedClockOut ?? '-' }}</span>
                        @else
                        <input type="text" class="data__input-time" name="requested_clock_in" value="{{ old('requested_clock_in',$requestedClockIn) }}" {{ $isLocked ? 'readonly' : '' }}>
                        <span class="tilde">〜</span>
                        <input type="text" class="data__input-time" name="requested_clock_out" value="{{ old('requested_clock_out', $requestedClockOut) }}" {{ $isLocked ? 'readonly' : ''}}>
                        @endif
                        @error('requested_clock_in')
                        <div class="attendance-detail__error">{{ $message }}</div>
                        @enderror
                        @error('requested_clock_out')
                        <div class="attendance-detail__error">{{ $message }}</div>
                        @enderror
                    </td>
                </tr>
                @foreach ($breaks as $break)
                <tr class="attendance-detail__table-row break-row">
                    <th class="detail-label">{{ $loop->index === 0 ? '休憩' : '休憩' . ($loop->index + 1) }}</th>
                    <td class="detail-data">
                        @if ($isPending || $viewReadOnly)
                        <span class="time-display data__input-time">{{ data_get($break, 'requested_break_start', '-') }}</span>
                        <span class="tilde">〜</span>
                        <span class="time-display data__input-time">{{ data_get($break, 'requested_break_end', '-') }}</span>
                        @else
                        <input type="text" class="data__input-time" name="breaks[{{ $loop->index }}][requested_break_start]" value="{{ data_get($break, 'requested_break_start', '') }}" {{ $isLocked ? 'readonly' : '' }} oninput="maybeAddRow({{ $loop->index }})">
                        <span class="tilde">〜</span>
                        <input type="text" class="data__input-time" name="breaks[{{ $loop->index }}][requested_break_end]" value="{{ data_get($break, 'requested_break_end', '') }}" {{ $isLocked ? 'readonly' : '' }} oninput="maybeAddRow({{ $loop->index }})">
                        @endif
                        @if ($errors->has("breaks.{$loop->index}"))
                        <div class="attendance-detail__error">
                            {{ $errors->first("breaks.{$loop->index}") }}
                        </div>
                        @endif
                        @foreach ($errors->get('breaks.'.$loop->index.'.requested_break_start') as $msg)
                        <div class="attendance-detail__error">{{ $msg }}</div>
                        @endforeach
                        @foreach ($errors->get('breaks.'.$loop->index.'.requested_break_end') as $msg)
                        <div class="attendance-detail__error">{{ $msg }}</div>
                        @endforeach
                    </td>
                </tr>
                @endforeach
                <tr class="attendance-detail__table-row textarea">
                    <th class="detail-label">備考</th>
                    <td class="detail-data">
                        @if ($isPending || $viewReadOnly)
                        <p class="note-display">{{ $requestNote }}</p>
                        @else
                        <textarea name="request_note" id="" class="detail-data__text" cols="15" rows="3" {{ $isLocked ? 'readonly' : ''}}>{{ $requestNote }}</textarea>
                        @endif
                        @error('request_note')
                        <div class="attendance-detail__error">{{ $message }}</div>
                        @enderror
                    </td>
                </tr>
            </tbody>
        </table>
        <div class="attendance-detail__action">
            @if ($viewReadOnly)
            <button class="action-button badge-approved" type="button" disabled>承認済み</button>
            @elseif ($isPending)
            <p class="pending-message">*承認待ちのため修正はできません。</p>
            @else
            <button class="action-button" type="submit">修正</button>
            @endif
        </div>
    </form>
</div>
@endsection

@section('scripts')
<script>
    window.isPending = @json($isPending);
    window.isLocked = @json($isLocked);
    window.breakIndex = @json(max(1, $breakCount));
</script>
<script src="{{ asset('js/attendance-detail.js') }}"></script>
@endsection