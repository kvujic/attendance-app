<div>
    ...
    <table class="attendance-list__table">
        ...
        <tbody class="attendance-list__table-body">
            ...
            <tr class="attendance-list__table-row row-body">
                ...
                <td class="list-data">
                    {{ $attendance?->total_break_time !== null
                            ? (int)($attendance->total_break_time / 60) . ':' . str_pad($attendance->total_break_time % 60, 2, '0', STR_PAD_LEFT)
                            : '' }}
                </td>
                <td class="list-data">
                    {{ $attendance?->total_work_time !== null
                            ? (int)($attendance->total_work_time / 60) . ':' . str_pad($attendance->total_work_time % 60, 2, '0', STR_PAD_LEFT)
                            : '' }}
                </td>
    ...
    <div class="csv-button__wrapper">
        <a class="csv-button" href="{{ route('admin.attendance.csv', [ 'user' => $user->id, 'month' => $currentMonth->format('Y-m'),]) }}">CSV出力</a>
    </div>
</div>